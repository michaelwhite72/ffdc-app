...
export const TENANT = process.env.TENANT || 'sandbox';
export const AUTHORIZATION_WELLKNOWN =
  process.env.AUTHORIZATION_WELLKNOWN || `${FFDC_URL}/login/v1/${TENANT}/.well-known/openid-configuration`;
...
if (!CLIENT_ID) {
  console.error('No client id. Set CLIENT_ID environment variable.');
  process.exit(1);
}

if (!CLIENT_SECRET) {
  console.error('No client secret. Set CLIENT_SECRET environment variable.');
  process.exit(1);
}
